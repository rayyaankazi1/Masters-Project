...
Config
------
  Adjust NUM_TOPICS_GRID, FISCAL_TOPIC_IDS, IDEOLOGY_TOPIC_IDS, and
  SKIP_TRAINING below.  See the WORKFLOW comment near FISCAL_TOPIC_IDS
  for step-by-step instructions on the inspect-then-assign workflow.
"""
...
# ── Training mode ─────────────────────────────────────────────────────────────
# SKIP_TRAINING = True  →  load saved model from data/interim/lda/ and jump
#                          straight to topic assignment + paragraphs_lda.csv.
#                          Use this after you have inspected topics and set the
...
#
# WORKFLOW:
#   1. First run: set FISCAL_TOPIC_IDS = [], SKIP_TRAINING = False
#      → trains, prints all topics, saves lda_topic_inspection.txt, exits.
#   2. Inspect outputs/tables/lda_topic_inspection.txt and lda_topics.png.
#   3. Set IDs below, set SKIP_TRAINING = True, re-run.
#      → loads saved model, writes paragraphs_lda.csv in ~2 min.
FISCAL_TOPIC_IDS: list[int] = [4, 8]

# ── Ideology topic IDs ────────────────────────────────────────────────────────
# Topics capturing ideological framing (libertad, estado, derechos, libre mercado).
...
def run(
    ...
    if SKIP_TRAINING:
        # ── 2–4. Load saved model (skip grid search + training) ───────────────
        model_path = os.path.join(LDA_DIR, "lda_model")
        dict_path  = os.path.join(LDA_DIR, "dictionary")
        corp_path  = os.path.join(LDA_DIR, "corpus.pkl")
        missing = [p for p in (model_path, dict_path, corp_path) if not os.path.exists(p)]
        if missing:
            raise FileNotFoundError(
                f"SKIP_TRAINING=True but saved files not found:\n"
                + "\n".join(f"  {p}" for p in missing)
                + "\nSet SKIP_TRAINING=False to run a full training pass first."
            )
        print("\nSKIP_TRAINING=True — loading saved model...")
        lda        = LdaModel.load(model_path)
        dictionary = corpora.Dictionary.load(dict_path)
        with open(corp_path, "rb") as f:
            corpus = pickle.load(f)
        best_k = lda.num_topics
        print(f"  Loaded k={best_k} model from {LDA_DIR}/")
    else:
